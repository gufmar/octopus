...
const brokenLinksObserver = new Proxy(brokenLinks, {
    set: function(target, key, value) {
        ...
        // report logging
        config['report-file'] && reportStream.write(JSON.stringify({
				"link": {
...
const request = async (requestUrl, referenceUrl, requestCallback) => {
    ...
    try {
        ...
        const response = await got( encodedUrl, {
            ...
            headers: {
                'user-agent': config['user-agent'] || 'Octopus'
            }
        } );
...
module.exports = (argv) => {
    ...
    config = {
        ...
        'report-file': String(argv['report-file']),
        'user-agent': String(argv['user-agent']),
    };
	...
	console.log('Crawling %s ...', baseHost); 
	
	// may create the report file
	if (config['report-file'] ) {
		console.log('write report to: %s', config['report-file']); 
		reportStream = fs
			.createWriteStream(config['report-file'], {flags:'a'})
			.on('error', function(err) {
				console.error('[report file writing error] : ' + err);
			});
	}
